...
- **后台**：接入后端数据库API，支持资产/认购管理、资产配售查询结算、生成对账报表功能。 
- **门户**：接入资产目录和资产端运营API，支持推荐资产和自定义查询、收益率前沿分析、认购、持仓战略推荐。
- **对账**：后台用户请求后可自动将USDC 收款地址归集 + 交易哈希对账导出CSV、Excel、PDF等多种格式，同时可增减必要信息  
- **监控**：用户请求下依照预设条件和算法自动进行全系统监控和风险提示
